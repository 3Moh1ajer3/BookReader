...
export const CHAPTER_4: Chapter = {
  id: "ch-4",
  title: "Chapter 4: Binary Taxonomy",
  readingTimeMinutes: 54,
  content: `# PART II: REVERSE ENGINEERING
## 4: BINARY TAXONOMY

> If you look down on one side, everything seems reassuringly familiar. . . . On the other side, it seems completely alien territory. —Mary Beard, SPQR
...
For reverse engineering, instead of popping your binaries into Ghidra or IDA Pro and tackling assembly code head-on, you should first learn to triage and select interesting binaries for further analysis. Not all binaries are

In this chapter, you’ll learn about three common categories of binaries: scripts, intermediate representations (IRs) such as bytecode, and machine code. You will then reverse engineer examples from each category. In addition, you’ll venture deeper into several subcategories of these binaries, which require different approaches. Beyond Executable Binaries and Shared Libraries Understanding the different types of binaries helps you select the right tools and techniques to reverse engineer them. By breaking them down into a few broad categories, you’ll be able to quickly triage your target and optimize your approach.
...
\`\`\`bash
$ curl -ohttps://raw.githubusercontent.com/nvm-sh/nvm/v0.40.0/install.sh | bash
$ source ~/.zshrc
$ nvm install --lts
$ npm install -g asar
...
In the case of DbGate, the developer has not enabled source maps for Webpack but has done so for two Rollup output files, query-parser-worker.js and bundle.js, as shown in Listing 4-2. rollup.config.js export default [
...
In the original TypeScript, the source code uses the import keyword to import dependencies ❶, but this is supported only in newer versions of JavaScript, such as ECMAScript 6. In addition, it includes type annotations that specify variable types ❷, which are not natively supported in JavaScript. getFilterType.js "use strict";
...
In cases where the TypeScript hasn’t been transpiled to JavaScript, you may still notice some subtle differences. For example, study the original code from packages/web/src/clientAuth.ts in Listing 4-6. clientAuth.ts import { apiCall, enableApi } from './utility/api';
...
The code uses the async keyword to define an asynchronous function ❶. Asynchronous functions return a Promise that allows the program to call the function but continue executing and responding to other events. In comparison, the code output/src/clientAuth.ts in Listing 4-7 looks somewhat different. clientAuth.ts import { __awaiter } from "tslib";
...
The beautified code reveals a fairly consistent structure of a list of function definitions. You may even spot code similar to the files you unpacked using the source map earlier, because the serverand client-side code share some common imported functions. One of these is compileMacroFunction:
...
This appears to be a viable sink-to-source path, but it isn’t a particularly exciting one. After all, if a user needs to enter the macro payload and click a button themselves to actually trigger this, then it’s more like a self-inflicted code execution requiring significant user interaction. Nevertheless, this might be a good place to start digging deeper for similar vulnerable code patterns. Reverse Engineering a Python Application In addition to Node.js Electron applications, applications in other programming languages, such as Python and Ruby, can also be bundled into executables. After all, one big advantage of scripting languages is portability; you need only a compatible interpreter to run most scripts on any platform. Electron applications are the most common, but it’s still useful to understand how to unpack some of these other types of applications, such as PyInstaller executables.

PyInstaller allows developers to bundle Python applications into a single package, such as a single-file executable. After executing the binary, PyInstaller starts a bootloader that unpacks compiled Python scripts (.pyc) and native libraries before running the main script with the bundled Python interpreter. This package is constructed in a fairly standard way, including a

*Table of Contents list and archive files.*
...
As with other bundled script-based executables, you can usually identify a PyInstaller executable by reviewing the strings or headers:
...
You can confirm whether it’s a PyInstaller executable using PyInstaller’s built-in pyi-archive_viewer utility to examine the CArchive of PyInstaller- Attack, a simple PyInstaller game with a Windows executable. Download main.exe and the source code from the GitHub release page (https://github .com/Amegma/Galaxy-Attack/releases/tag/v1.3.0). Next, install PyInstaller and run the archive viewer utility:
...
Although actual software distributed as PyInstaller executables is much rarer than Electron applications, working on this simple example helps illustrate some common patterns in reverse engineering software written in scripting languages. It’s impossible to completely remove the presence of source code, even if it’s been compiled, transpiled, or bundled in some way. However, the degree of lossiness can have a significant impact on the ease of analysis. Intermediate Representations In terms of abstraction, intermediate representations lie between machine code and the source code. As the name suggests, these are higher-level representations of source code that can be interpreted and executed by a runtime.
...
Finally, another characteristic of intermediate representation binaries is that they usually include additional metadata that affects their runtime environment configuration. For example, the Java Archive (JAR) package file format includes a manifest that tells the JVM which class corresponds to the application entry point, what dependencies are required, and other important information. Similarly, .NET binaries, also known as assemblies, include a manifest containing metadata such as version numbers, included files, and references. Assemblies also include metadata about every type and member they use, which is extremely useful for decompilation.
...
Type metadata Metadata tables that define the types and members

used in the assembly

CIL code The actual intermediate language code that is executed in

the CLR

Resources Assets such as images, configuration, and other data

Strong name signature An optional digital signature to verify the

assembly

You can explore this by analyzing LiteDB Studio, a graphical interface for viewing and editing LiteDB database files. Since the executable was compiled for Windows and the tools used to reverse engineer it are primarily Windows-based, you should perform the steps described here on Windows if possible. If that’s not an option, it is possible to run the tools on other platforms, with varying degrees of difficulty.

Download the LiteDB Studio binary from https://github.com/mbdavid/

### LiteDB.Studio/releases/download/v1.0.3/LiteDB.Studio.exe. You can use the

PE-Bear tool to view some of the properties of the assembly; download the latest release from https://github.com/hasherezade/pe-bear/releases.
...
If you haven’t already installed Visual Studio on Windows, install it with the .NET Framework tools to access the IL Disassembler. Once installed, you should be able to run it with ildasm.exe in the Visual Studio Developer Command Prompt. As a quick test, compile the C# code in Listing 4-9 in Visual Studio using the Console App (.NET Framework) template. Program.cs using System;
...
{
.method public hidebysig static voidMain(string[] args) cil managed ❺
{
.entrypoint
...
One of the most useful features of ILSpy is the Analyze function, which you can access by right-clicking any member name. This brings up a tree containing the other members that use or are used by it, which is especially useful for sink-to-source analysis. For example, if you identify LiteDB.Studio .MainForm.ExecuteSql as a potential vulnerable sink, you can use the Analyze feature to find out that it’s used by five other methods. You can then follow the nested Used By tree until you reach a suitable ancestor.

Of course, you aren’t restricted to ILSpy’s user interface. You can also right-click the assembly in the left sidebar and select Save Code to export the decompiled source code. From there, you can run automated code analysis tools or perform manual code review. You can also open the source code in an IDE that will provide similar analysis tools to ILSpy. Other decompilers, like JetBrains, dotPeek, and dnSpyEx, also come with debuggers to perform dynamic analysis of .NET assemblies. Java Bytecode Similar to the .NET Framework’s CIL, Java also uses an intermediate representation that gets executed by a common runtime—in this case, the JVM platform. Like CIL, Java bytecode uses a higher-level instruction set than machine code, but unlike CIL, Java bytecode also uses registers in the form of a local variable array. In practice, you’ll most often encounter Java binaries distributed as Java Archive files with the.jar extension.

Like .NET assemblies, JAR files bundle together bytecode (Java class files), resources, and metadata into a single file. However, while the PE forbe unpacked with any archive extraction program. This can make executing them a little less intuitive, as they must be run with the Java executable instead of executing them directly.
...
Place the decompiler JAR file (rename it java-decompiler-engine.jar) and pixelwheels.jar in the same directory, then perform the decompilation with the following commands:
...
Without particular expertise in reading Java bytecode, you can still match up the output to the original source code. For example, the File.mkdirs method is invoked followed by an ifne condition jump instruction ❶, which corresponds to the if-else conditional in the source code. Eventually, the #38 ❷, meaning the string has index 38 in the pool, and called with the String Builder.append method.

After decompiling the source code, you can analyze it by applying the code review strategies you learned in the previous chapters, with the caveat that you shouldn’t always take the decompiled output at face value. For example, if you perform attack surface analysis on the code, you may notice an interesting RemoteInput class in com/badlogic/gdx/input/RemoteInput.java that opens the default port 8190. However, this code is not actually used elsewhere in the application, possibly because the developer decided not to enable the remote play feature. Machine Code Machine code is the lowest-level abstraction among the three binary categories explored in this chapter. Like binaries in general, even machine code binaries are not created or compiled equally. Programming languages such as C++, Golang, and Rust compile to machine code in different ways, and these differences can significantly affect the ease of reverse engineering them.
...
To quickly compare machine code, assembly, and pseudocode, you can analyze a “Hello World” program written in C: hello-world.c #include <stdio.h>
...
40052f: e8 cc fe ff ff callq400400 <puts@plt>
400534: b8 00 00 00 00 mov$0x0,%eax
400539: 5d pop%rbp
40053a: c3 retq
40053b: 0f 1f 44 00 00 nopl0x0(%rax,%rax,1)
\`\`\`
...
In the next sections, we’ll quickly examine how different compilation methods affect the difficulty of reverse-engineering machine code binaries. Statically Linked A statically linked binary is compiled with all the libraries it uses, instead of loading external libraries from the system at runtime. There are several advantages and disadvantages to this approach. On one hand, it makes the binary portable, since it can be executed independently without depending on external libraries to be installed on the operating system. On the other hand, it creates a much larger binary because more machine code must be included in the output.

You can test this with a Golang implementation of “Hello World,” since Golang compiles statically linked binaries by default: hello-world.go package main
...
*table, you’ll get no results because there are no dynamically linked functions.*

Instead, you need to dump the whole symbol table to get the statically linked functions that are now part of the binary:

\`\`\`text
$ objdump -t hello-world
hello-world: file format elf64-x86-64
SYMBOL TABLE:
0000000000000000 ldf *ABS* 0000000000000000 go.go
...
While the binary does the exact same thing as the C “Hello World” example, the machine code produced by the Golang compiler is harder for Ghidra to decipher. This is because Go binaries are compiled with the Go runtime, which performs additional functions such as garbage collection and stack management. Additionally, you may notice that the final output includes fmt.Fprintln instead of Println ❶. This is because in the fmt package, Println is a wrapper around the Fprintln function, so the compiler optimizes it away, similar to what happened with printf and puts earlier. Dynamically Linked In contrast to statically linked binaries, dynamically linked binaries are compiled with information about the libraries they depend on, but not the actual libraries themselves. The operating system parses this information and loads the libraries in memory at runtime. As a quick comparison, check the dynamic symbol table for the C “Hello World” binary using the dynamic

\`\`\`bash
$ objdump -T hello-world
hello-world: file format elf64-x86-64
DYNAMIC SYMBOL TABLE:
0000000000000000 DF *UND* 0000000000000000GLIBC_2.2.5 puts
0000000000000000 DF *UND* 0000000000000000GLIBC_2.2.5 __libc_start_main
0000000000000000wD*UND* 0000000000000000 __gmon_start__
\`\`\`
...
------------------------------------------------
1850090 ->114632061.96%linux/amd64hello-world-packed
Packed 1 file.
\`\`\`
...
The most important step in dealing with a packed binary is to first recwas used. In the case of UPX, the initial instructions are well known, and UPX helpfully includes the magic bytes 0x55505821 (“UPX!” in ASCII) in the header. You can observe this in a simple hex dump of the packed binary:

\`\`\`text
> hexdump -C hello-world-packed | head -n 20
000000007f 45 4c 46 02 01 01 0000 00 00 00 00 00 00 00|.ELF............|
0000001002 00 3e 00 01 00 00 0008 33 5e 00 00 00 00 00|..>......3^.....|
0000002040 00 00 00 00 00 00 0000 00 00 00 00 00 00 00|@...............|
0000003000 00 00 00 40 00 38 0003 00 00 00 00 00 00 00|....@.8.........|
0000004001 00 00 00 06 00 00 0000 00 00 00 00 00 00 00|................|
0000005000 00 40 00 00 00 00 0000 00 40 00 00 00 00 00|..@.......@.....|
0000006000 10 00 00 00 00 00 00d0 e7 15 00 00 00 00 00|................|
0000007000 10 00 00 00 00 00 0001 00 00 00 05 00 00 00|................|
0000008000 00 00 00 00 00 00 0000 f0 55 00 00 00 00 00|..........U.....|
0000009000 f0 55 00 00 00 00 00e6 4d 08 00 00 00 00 00|..U......M......|
000000a0e6 4d 08 00 00 00 00 0000 10 00 00 00 00 00 00|.M..............|
000000b051 e5 74 64 06 00 00 0000 00 00 00 00 00 00 00|Q.td............|
000000c000 00 00 00 00 00 00 0000 00 00 00 00 00 00 00|................|
*
000000e008 00 00 00 00 00 00 004f 05 91 f3 55 50 58 21|........O...UPX!|
000000f0ec 0a 0e 16 00 00 00 00ea 3a 1c 00 6a 6c 09 00|.........:..jl..|
00000100c8 01 00 00 9d 00 00 0008 00 00 00 bb fb 20 ff|.............. .|
000001107f 45 4c 46 02 01 01 0002 00 3e 00 1b 80 ed 45|.ELF......>....E|
000001201f bf 5f da ed 40 2f c845 26 38 00 07 0a 17 00|.._..@/.E&8.....|
0000013003 3e d8 d7 de 00 06 1e04 4f 40 00 40 0f 88 01|.>.......O@.@...|
\`\`\`

Fortunately, UPX allows you to easily unpack UPX-packed binaries via the -d command line option (enter upx -d hello-world-packed to try it for yourself). Some packers and obfuscators deploy techniques that make it difficult to reverse engineer, such as encrypting data with randomized values, and may require you to dump the unpacked and decrypted bytes from memory or analyze the unpacking routine in detail. While you’ll encounter this more commonly with malware, it helps to be prepared to recognize a situation where a packer has been used and to know how to approach such binaries. Summary In this chapter, you navigated a wide range of binaries across different categories, including scripts, intermediate representations, and machine code. You also reverse engineered simple examples of each type with appropriate tools and techniques.

As you target larger and more complex software, like firmware, you may need to juggle multiple types of binaries. For example, an Android application written in JavaScript (React Native) may call a native module compiled from Java, which in turn can call C++ libraries via the Java Native Interface. This is why it’s essential to build breadth rather than focus too much on

While this was hardly an exhaustive introduction to all the types of binaries you’ll encounter, you should be able to generalize some of the approaches used regardless of the programming language or compilation. For example, keep an eye out for sources of metadata that can help you analyze the machine code more effectively or even decompile it to source code. Pay attention to languageor compiler-specific quirks and optimizations that can assist you in identifying function names and imports. Look for clues that a developer has used a packer or obfuscator, identify the tool used, and read the documentation to find out how to reverse it, if possible. These tips will help you identify the most important or useful parts of the program to reverse engineer first, which is a topic the next chapter will explore in greater detail.`,
};
